...
const ReplayModal: React.FC<ReplayModalProps> = ({ isOpen, onClose, replays }) => {
  ...
  return (
    ...
    >
      <div 
        ref={modalRef}
        className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden border border-gray-100 transform transition-all duration-300 scale-95 hover:scale-100"
      >
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white">
          <h2 className="text-2xl font-bold text-gray-800">Chef Responses</h2>
          <button 
            onClick={onClose}
        ...
        {/* Content */}
        <div className="overflow-y-auto flex-grow p-6 bg-gradient-to-b from-white to-gray-50">
          {sortedReplays.length > 0 ? (
            <div className="space-y-4">
              {sortedReplays.map(replay => (
                <ReplayCard key={replay._id} replay={replay} />
            ...
            <div className="flex flex-col items-center justify-center h-64 text-gray-400">
              ...
              <p className="text-lg font-medium">No chef responses yet</p>
              <p className="text-sm mt-1 text-gray-500">Chefs will appear here when they respond to your post</p>
            </div>
          )}
        </div>
      </div>
