...
const ProfileActions = () => {
  ...
  return (
    <div className="px-6 py-6 bg-gray-50 rounded-lg">
      <h2 className="text-lg font-medium text-[#2D4F2B] mb-4">Account Settings</h2>

      <div className="flex flex-wrap gap-3">
        <button
          className="flex items-center px-4 py-2.5 bg-white border border-[#D1E8CE] rounded-lg 
                     text-sm font-medium text-[#2D4F2B] hover:bg-[#F1F8E9] focus:outline-none 
                     focus:ring-2 focus:ring-offset-2 focus:ring-[#708A58] transition-all duration-200"
          onClick={() => setEditModalOpen(true)}
        >
          <Edit size={16} className="mr-2 text-[#708A58]" />
          Edit Profile
        </button>
        ...
        {user?.isGoogleUser && user?.hasPassword === false && (
          <button
            ...
            onClick={() => setSetPasswordModalOpen(true)}
          >
            <KeyRound size={16} className="mr-2 text-[#708A58]" />
            Set Password
          </button>
        )}
        ...
        <button
          ...
          onClick={handleLogout}
        >
          <LogOut size={16} className="mr-2 text-[#708A58]" />
          Sign Out
        </button>
      </div>
